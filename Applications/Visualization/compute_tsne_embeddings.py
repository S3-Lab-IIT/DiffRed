...
def parse_arguments():
    ...
    parser.add_argument('--save_dir', '-s', help='Path to the directory where embeddings are to be saved', default='./tsne_embeddings/')
...
def compute_embeddings(dataset:str, DATA_DIR:str,SAVE_DIR:str, dr_tech:str, target_dim:int, k1:int, k2:int, max_iter:int, setting:dict, tsne:str, scaling:bool):
    ...
    if dr_tech=='DiffRed':
        ...
        if tsne=='cuda':
            Z=DR_MAPS['T-SNE'](X,SETTINGS['T-SNE'][setting])
        elif tsne=='multicore':
...
def main():
    ...
    if args.dr_tech=='DiffRed':
            ...
            results=[pool.apply_async(compute_embeddings, args=(args.dataset, args.data_dir, args.save_dir,args.dr_tech,target_dims[i], k1[i], k2[i], max_iter[i], args.setting, args.tsne )) for i in range(len(k1))]

    else:

        results=[pool.apply_async(compute_embeddings, args=(args.dataset, args.data_dir, args.save_dir, args.dr_tech, target_dims[i], None, None, None, args.setting, args.tsne)) for i in range(len(target_dims)) ]
